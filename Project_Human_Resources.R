...
glimpse(hr_train)
sum(promotion_last_5years) # 228

# Find out the variance in statisfaction_level for category 0 of variable 'left' 
#(round off to 4 decimal places) -
tp <- dplyr::filter(hr_train, left == "0")
var(tp$satisfaction_level) # 0.04868534

# Does average_monthly_hours follow normal distribution -
qqnorm(average_montly_hours) # No

# Which category of salary has maximum employee resignation - 
table(salary) # low
salh <- dplyr::filter(hr_train, salary == "high" & left == "1")
nrow(salh)
sall <- dplyr::filter(hr_train, salary == "low" & left == "1")
nrow(sall)
salm <- dplyr::filter(hr_train, salary == "medium" & left == "1")
nrow(salm)

# correlation coefficient between last_evaluation and average_monthly_hours 
cor(last_evaluation, average_montly_hours) # 0.333951

# what is the probability that someone will leave the organisation 
#if they were involved in a work accident - 0.48



# median time spent with the company among people leaving the company -
temp <- dplyr::filter(hr_train, left == "1")
median(temp$time_spend_company)  # 3

# Which sales category has maximum median average_monthly_hours -
table(sales)
sort(by(average_montly_hours, sales, median), decreasing = T) # accounting

# Does number of projects significantly differ between 
#two categories of the target variable "left" -
